...
class GraphFlowModel(nn.Module):
    ...
    def forward(self, inp_node_features, inp_adj_features):
        ...
        Returns:
            ...
            logdet:  ([B], [B])        
        """
        # inp_node_features = torch.zeros(inp_node_features.shape)
        ...
        inp_node_features_cont += self.deq_coeff * torch.rand(inp_node_features_cont.size(), device=inp_node_features_cont.device) #(B, N, 9)
    ...
    def generate(self, atom_list, temperature=0.75, min_atoms=7, max_atoms=48):
        ...
        with torch.no_grad():
            ...
            # print('num2bond', num2bond)
            # print('num2atom', num2atom)
            # print(self.node_dim)
            if self.dp:
            ...
            for i in range(max_atoms):
                ...
                for j in range(edge_total):
                    ...
                    while not valid:
                        ...
                        # if True: # haven't sampled all possible bond type or is not stuck in the loop
                        ...
                        # else:
                            ...
                            # edge_discrete_id = 1 # we have no choice but to choose not to add edge between (i, j+start) 3
                        # try
                        # print('add adj')
                        # edge_discrete_id = 0
                        ...
                        # if edge_discrete_id == 1: # virtual edge
                        valid = True
    ...
    def log_prob(self, z, logdet):
        logdet = logdet - self.latent_edge_length # calculate probability of a region from probability density, minus constant has no effect on optimization
        ll_edge = -1/2 * (torch.log(2 * self.constant_pi) + self.prior_ln_var + torch.exp(-self.prior_ln_var) * (z**2))
        ll_edge = ll_edge.sum(-1) # (B)
        # print(logdet)
        ll_edge += logdet #([B])
        
        return -(torch.mean(ll_edge) / (self.latent_edge_length ))
