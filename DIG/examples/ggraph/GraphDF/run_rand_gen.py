...
from dig.ggraph.dataset import QM9, ZINC250k, MOSES, DD
...
if args.data == 'qm9':
    ...
    with open('config/DD.json') as f:
        conf = json.load(f)
    dataset = DD(conf_dict=conf['data'], one_shot=False, use_aug=True)  
else:
    print("Only qm9, zinc250k, dd, and moses datasets are supported!")
    exit()
